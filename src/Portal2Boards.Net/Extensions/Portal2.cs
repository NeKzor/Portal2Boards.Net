...
namespace Portal2Boards.Extensions
{
	public static class Portal2
	{
		...
		public static IReadOnlyCollection<Map> SinglePlayerMaps = CampaignMaps
			.Where(m => m.Type == MapType.SinglePlayer)
			.ToList();
		public static IReadOnlyCollection<Map> CooperativeMaps = CampaignMaps
			.Where(m => m.Type == MapType.Cooperative)
			.ToList();

		public static float? AsTime(this uint? time)
			=> (time != default(uint?)) ? (float)Math.Round((float)time / 100, 2) : default;
		public static float AsTime(this uint time)
			=> (time != default(uint)) ? (float)Math.Round((float)time / 100, 2) : default;
		...
		public static Map GetMapByName(string name, StringComparison comparison = StringComparison.CurrentCultureIgnoreCase)
			=> CampaignMaps
				.FirstOrDefault(m => string.Equals(m.Name, name, comparison) || string.Equals(m.Alias, name, comparison));
		public static Map GetMapById(ulong id)
			=> CampaignMaps
